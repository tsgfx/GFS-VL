...
def filter_captions_by_interval(caption_info, caption_idx_info):
    for idx, scene_caption_idx_info in tqdm.tqdm(enumerate(caption_idx_info), total=len(caption_idx_info)):
        ...
        for image_name in image_name_list:
            real_image_name = int(image_name.split('_')[0])
            
            if real_image_name % args.interval != 0:
...
if __name__ == '__main__':
    ...
    filter_captions_by_interval(caption_info, caption_idx_info)
